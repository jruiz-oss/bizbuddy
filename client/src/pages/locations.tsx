...
function FitBounds({
  ...
  selectionIds: string[];
  // Bumped by the "Fit to selection" button so the fit effect re-runs even
  // when the selection itself hasn't changed.
  fitNonce: number;
}) {
  const map = useMap();
...
export default function Locations({ selectedClientId, setSelectedClientId }: LocationsProps) {
  ...
  // FitBounds owns the actual map math; bumping the nonce re-triggers it so the
  // button works for a multi-pin selection and for repeat clicks.
  const fitToSelection = () => setFitNonce((n) => n + 1);

  const renderBulkActions = () => {
    const count = selectedPinIds.size;
    if (count === 0) return null;
    const canRemoveFolder = folderFilter !== "all" && folderFilter !== "hidden";
    const rows: Array<{ key: string; label: string; icon: React.ReactNode; onClick: () => void; testid: string }> = [
      { key: "hours", label: "Update hours", icon: <Clock className="w-4 h-4" />, onClick: () => openBulkAction("hours"), testid: "button-bulk-hours" },
      { key: "post", label: "Create post", icon: <MessageSquarePlus className="w-4 h-4" />, onClick: () => openBulkAction("post"), testid: "button-bulk-post" },
      { key: "photo", label: "Upload photo", icon: <ImagePlus className="w-4 h-4" />, onClick: () => openBulkAction("photo"), testid: "button-bulk-photo" },
      { key: "folder", label: "Add to folder", icon: <Folder className="w-4 h-4" />, onClick: () => openBulkAction("folder"), testid: "button-bulk-folder" },
      { key: "tag", label: "Add tag", icon: <TagIcon className="w-4 h-4" />, onClick: () => openBulkAction("tag"), testid: "button-bulk-tag" },
    ];
    return (
      <div className="space-y-3" data-testid="nearby-selection-toolbar">
        <div className="flex items-center justify-between">
          <span className="text-xs font-semibold uppercase tracking-wide text-gray-500" data-testid="text-selection-count">
            {count} selected
          </span>
          <button
            type="button"
        ...
        <div className="space-y-2">
          {rows.map((r) => (
            <button
              ...
              className="w-full flex items-center gap-3 rounded-lg border border-gray-200 px-3.5 py-3 text-sm font-medium text-gray-800 hover:bg-gray-50 hover:border-gray-300 transition-colors"
            >
          ...
          {canRemoveFolder && (
            <button
              ...
              data-testid="button-bulk-remove-folder"
              className="w-full flex items-center gap-3 rounded-lg border border-red-200 px-3.5 py-3 text-sm font-medium text-red-600 hover:bg-red-50 transition-colors"
            >
          ...
          <button
            ...
            data-testid="button-fit-selection"
            className="w-full flex items-center gap-3 rounded-lg px-3.5 py-3 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors"
          >
            <Maximize2 className="w-4 h-4" />
            <span className="flex-1 text-left">Fit to selection</span>
          </button>
        </div>
      </div>
    );
...
function SelectedCard({
  ...
  return (
    <div className="p-5" data-testid="selected-card">
      ...
      {bulkActions && (
        <div className="mt-5 pt-5 border-t border-gray-200">{bulkActions}</div>
      )}
    </div>
  );
}
